...
public interface RelaxationTechniquesRepository extends CrudRepository<RelaxationTechniqueAudios, Integer>{
    ...
    @Query(value = "Select audioId FROM relaxationtechniqueaudios where audioId=:audioId and auditory LIKE CONCAT('%', :auditory, '%')", nativeQuery = true)
    ...
    @Query(value = "Select url from relaxationtechniquesaudios where audioId = :audioId", nativeQuery = true)
